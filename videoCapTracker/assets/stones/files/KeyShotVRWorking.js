...
window.keyshotVR = function(U, v, V, W, pa, D, qa, ra, sa, ta, ua, J, K, X, Y, va, Z, wa, $, xa, aa, ya, za, Aa, Ba, p) {
    ...
    function Da(f) {//nothing happened
        a.ma = z;
        f.pointerId in a.F && delete a.F[f.pointerId];
        ca(f)
    }

    function Ea(f) {//nothing happened
        a.ma && (a.F[f.pointerId] = [f.pageX, f.pageY], da(f))
    }

    function Fa(f) {//nothing happend
        a.ma = r;
        a.F[f.pointerId] = [f.pageX, f.pageY];
        ea(f)
    }

    function fa() {//on mousemove
    ...
    function M() {//pooppooppoop
        a.Sa(a.h.offsetWidth, a.h.offsetHeight)
    }

    function ha(a, d) {//onmousedown
        l.start.x = a;
        l.start.y = d;
        l.a.x = a;
        l.a.y = d;
        ...
        if (1 == d.length && d[0])
            if (j.start.x = d[0].pageX, j.start.y =
                ...
                for (var e = a.pa; e && e != a.r;) {
                    if (e.onmouseenter) e.onmouseenter();
                    e = e.parentNode
                }
    ...
    function Ga(a) {//nothing
        a.preventDefault();
        j.d = 0
    }

    function S() {//infinite poop
        Ha(S);//more questions
    ...
    var R, Q = t;
    this.va = function(b) {//onmousedown still gets call if switched to mouseenter
        var d = G();
        ...
        1 == j.d ? ha(b.pageX, b.pageY) : 2 == j.d ? (E.start.x = a.cursor.x, E.start.y = a.cursor.y, c.t = a.v(), a.e = a.cursor.x - 0.5 * C.width, a.f = a.cursor.y - 0.5 * C.height, a.e *= a.R, a.f *= a.R, a.e -= c.j, a.f -= c.k, a.e /= c.t, a.f /=
         ...
         if (test){alert(j.d); test=false;}

    };
    this.Ga = function(b) {//nada
    ...
    this.Sa = function(b, d) {//pooppooppoop
    ...
    this.U = function(a) {//pooppooppoop
        m(a, "mouseenter", F);
        m(a, "mousemove", H);
        m(a, "mouseout", N)
    };
    this.aa = function(a) {//pooppooppoop
        O(a, "mouseenter", F);
        O(a, "mousemove", H);
        O(a, "mouseout", N)
    };
    this.Qa = function() {//poop
        a.p && (a.p.style.visibility = "hidden",
            a.aa(a.p), a.aa(a.Q), a.aa(a.o))
    };
    this.Ka = function() {//nada
    ...
    this.ta = function() {//poop
        a.p = document.createElement("div");
        a.U(a.p);
        a.p.V = function() {//pooppooppoop
    ...
    this.Ra = function(b) {//pooppooppooppooppooppoop
        if (a.p) {
    ...
    this.sa = function() {//poop
        T = r;
        S()
    };
    this.ya = function() {//poop
        m(window, "resize", M);
        za && m(n, "dblclick", Ca);
        if (ba())
            ...
            } else m(n, "mouseenter", F), m(q, "mouseenter", F), m(n, "mousemove", H), m(q, "mousemove", H), m(document, "mouseout", N);
        else n.addEventListener && (m(document, "mouseout", N), m(n, "mousewheel", P), m(document, "keydown", la), m(n, "mouseenter", F), m(q, "mouseenter", F), m(n, "mousemove", H), m(n, "touchstart", ea),
            m(n, "touchmove", da), m(n, "touchcancel", Ga), m(n, "touchend", ca), m(n, "DOMMouseScroll", P), m(window, "orientationchange", fa))
    };
    if (document.createElement("canvas").getContext) {//poop
        var a = this,
            u = a.h = t,
            s = t,
            na = t,
